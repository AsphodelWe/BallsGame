...
public class GlobalUIManager : MonoBehaviour
{
    ...
    private void Initialize()
    {
        ...
        _ballRegistry.OnEnemyAdded += SetBallHealth;

    }

    private void UpdateHealthBar(int current, int max, VisualElement fill, Label text)
    {
        text.text = $"{current}/{max}";
        fill.style.width = Length.Percent((float)current / max * 100);
    }

    private void SetBallHealth(BallPresent ball)
    {
    ...
    private void UpdateBarsAlignment()
    {
